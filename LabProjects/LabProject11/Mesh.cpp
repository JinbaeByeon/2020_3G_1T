...
CCubeMeshDiffused::CCubeMeshDiffused(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth, float fHeight, float fDepth) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	//정점 버퍼는 직육면체의 꼭지점 8개에 대한 정점 데이터를 가진다. CDiffusedVertex pVertices[8];
...
CMapMesh::CMapMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth, float fHeight, float fDepth) :CMesh(pd3dDevice, pd3dCommandList)
{
	const int numSlice = 30;
	const int numPlane = numSlice * numSlice * 4;
	m_nVertices = numPlane + numSlice * 4;
	...
	CVertex* pVertices= new CVertex[numPlane + numSlice * 4];
	...
	for (int i = 0; i <= numSlice; ++i) {
		for (int j = 0; j <= numSlice; ++j) {
			idx = (i * numSlice * 4 + j);
			wM = (-1.f + (2.f * i) / numSlice);
			hM = (-1.f + (2.f * j) / numSlice);
			// Top
			pVertices[idx] = CVertex(XMFLOAT3(fx * hM, +fy, fz * wM));
			// Right
			pVertices[30 + idx] = CVertex(XMFLOAT3(+fx, fy * (-hM), fz * wM));
			// Bottom
			pVertices[60 + idx] = CVertex(XMFLOAT3(fx * (-hM), -fy, fz * wM));
			// Left
			pVertices[90 + idx] = CVertex(XMFLOAT3(-fx, fy * hM, fz * wM));
		}
	}
	...
	/*인덱스 버퍼는 맵의 3600개의 면(사각형)에 대한 기하 정보를 갖는다. 삼각형 리스트로 맵을 표현할 것이
므로 각 면은 2개의 삼각형을 가지고 각 삼각형은 3개의 정점이 필요하다. 즉, 인덱스 버퍼는 전체 (=3600*2*3)개의 인
덱스를 가져야 한다.*/
	m_nIndices = numPlane* 6;
	UINT* pnIndices = new UINT[numPlane * 6];
	...
	for (int i = 0; i < numSlice; ++i) {
		for (int j = 0; j < numSlice; ++j) {
			for (int k = 0; k < 4; ++k) {
				iVertex = numSlice * (i * 4 + k) + j; //0, 2, 4, 6 // 1,3,5,7 // 8, 10, 12, 14 // 9, 11, 13, 15 (4*numSlice)
				if ((iVertex + 1) % numSlice * 4 == 0) {
					pnIndices[idx++] = iVertex; pnIndices[idx++] = iVertex + numSlice * 4; pnIndices[idx++] = iVertex + 1 - numSlice * 4;
					pnIndices[idx++] = iVertex + 1 - numSlice * 4; pnIndices[idx++] = iVertex + numSlice * 4; pnIndices[idx++] = iVertex + 1;
				}
				else {
					pnIndices[idx++] = iVertex; pnIndices[idx++] = iVertex + numSlice * 4; pnIndices[idx++] = iVertex + 1;
					pnIndices[idx++] = iVertex + 1; pnIndices[idx++] = iVertex + numSlice * 4; pnIndices[idx++] = iVertex + numSlice * 4 + 1;
				}
			}
		}
	}
